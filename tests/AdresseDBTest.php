...
class AdresseDBTest extends TestCase {
    ...
    public function testAjout() {
        try {
         ...
         $a = new Adresse(99,32, "rue Jean moulin", 44000, "Nantes",4);
            $id_pers=4;
          $this->adresse->ajout($a, $id_pers);

          $adr = $this->adresse->selectAdresseIdPers($a->getId_personne());
    ...
    public function testSuppression() {
        try{
            ...
            $this->adresse = new AdresseDB($this->pdodb);
            $a = new Adresse(100,42, "rue du Moulin", 75000, "Paris",5);
    ...
    public function testUpdate() {
      ...
      $this->adresse = new AdresseDB($this->pdodb);
    $a=new Adresse(101,26, "boulevard victor Hugo",44000,"Nantes",5);
    $a->setId(66);
    $this->adresse->update($a);  
    $newAdr=$this->adresse->selectAdresse(66);
    $this->assertEquals($a->getId(),$newAdr->getId());
    $this->assertEquals($a->getNumero(),$newAdr->getNumero());
    $this->assertEquals($a->getRue(),$newAdr->getRue());
    $this->assertEquals($a->getCodePostal(),$newAdr->getCodePostal());
    $this->assertEquals($a->getVille(),$newAdr->getVille());
